...
class Table(seamm.Node):
    ...
    def description_text(self, P=None):
        ...
        text += "{method} table"
        return (
            self.header + "\n" + __(text, method=self.method, indent=4 * " ").__str__()
        )
    ...
    def run(self):
        ...
        if self.method == "create":
            ...
            if self.index_column != "default":
                ...
                table.set_index(index_column, inplace=True, verify_integrity=True)
            ...
            if file_format == "csv":
                ...
                raise RuntimeError(
                    "Table save: cannot handle format '"
                    + file_format
                    + "' for file '"
                    + filename
                )
            ...
            if self.column_index is None:
                raise RuntimeError("Table get element: the column index must be given")
            if self.row_index is None:
                raise RuntimeError("Table get element: the row index must be given")
